 ...
 * read file function
 **********************************************************************/
vector< std::complex<double> read_file(string tx_data)
{
    //create read buffer
    ...
    //open file
    std::ifstream is;
    is.open (tx_data, std::ios::binary | std::ios::in);
    ...
    double* pbuff = (double*)rbuff;
    std::vector<double> tmp_buff(pbuff, pbuff + (rlen / sizeof(double)));
    ...
    for ( int i = 0; i < tmp_buff.size(); i++) {
        ...
        std::complex<double> cp(real, imag);
        buff.push_back(cp);
    }
...
void transmit_worker(
    ...
    std::vector<std::complex<double> *> buffs(num_channels, &buff.front());
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    std::string tx_args, tx_ant, tx_subdev, ref, otw, tx_channels, tx_data;
    ...
    desc.add_options()
		...
		("tx-data", po::value<std::string>(&tx_data), "transmit data")
    ;
    ...
    //detect/parsing which channels to use
    //default chnnel: 0, input channel foramt(0,1,2 or 0\1\2).
    ...
    //Lock mboard(mother-board) clocks
    tx_usrp->set_clock_source(ref);
    rx_usrp->set_clock_source(ref);

    // setup tx/rx subdev if any of these parameter is given.
    ...
    //check the transmit sample rate
    ...
    /*
        ...
        The follwing CPU foramt have been implemented:
        * fc64
        * fc32 (default using)
        * sc16
        * sc8
    */
    /*
        otw stands for "over-the-wire sample mode"
        Setting the OTW ("over-the-wire") format is, in theory, transparent to the application, but changing this can have some side effects.
        Using less bits for example (e.g. when going from otw_format sc16 to sc8) will reduce the dynamic range, and increases quantization noise.
        On the other hand, it reduces the load on the data link and thus allows more bandwidth (a USRP N210 can work with 25 MHz bandwidth for 16-Bit complex samples, and 50 MHz for 8-Bit complex samples).
        The following otw format have been implemented:
         * sc16 (default using)
         * sc12
         * sc8
    ...
    */
	...
	transmit_thread.create_thread(boost::bind(&transmit_worker, buff, tx_stream, md, num_channels));
